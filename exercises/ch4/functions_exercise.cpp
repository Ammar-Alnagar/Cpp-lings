 ...
 * The program should calculate the factorial of a number and check if a number
 * is prime.
 */
...
int main() {
  ...
  if (number >= 0) {
    std::cout << "Fibonacci number at position " << number
              << " is: " << fib(number) << std::endl;
  } else {
    std::cout << "Fibonacci is not defined for negative positions."
              << std::endl;
  }
